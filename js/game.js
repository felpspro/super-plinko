...
function createCoins(){
	...
	for(var n=0; n<gameData.totalCoins; n++){
		if(n < gameData.coinArray.length){
			...
			gameData.coinPost.push({r:targetR, c:targetC, index:$.coins[n].prizeIndex});
...
function createCoinText(obj){
	...
	newText.text = textStrings.collectCoin.replace('[NUMBER]', coinValue);
	newText.x = obj.x;
	newText.y = obj.y;
...
function dropBall(target){
	...
	if(!gameSettings.gamePlayType && playerData.bet == 0 && !gameData.bonusRound){
		toggleGameStat(textStrings.minBet);
		return;
	}

	gameData.targetDrop = target;
	if(!gameData.bonusRound){
		if(gameSettings.gamePlayType){
			playerData.chance--;
		...
		if(typeof memberData != 'undefined' && memberSettings.enableMembership){
			if(!gameData.physicsEngine){
				var postData = {
					column:target.dropIndex,
					coin:gameData.coinPost,
					bet:playerData.bet,
					bonus:false,
				}
				getUserResult("proceedDropBall", postData);
			}else{
				var postData = {
					bet:playerData.bet,
					win:0,
				...
				updateUserPoint("proceedDropBall", postData);
			}
		}else{
			proceedDropBall();
		}
...
function proceedDropBall(result){
	gameData.path = [];
	if(result != undefined){
	...
	toggleGameStat(textStrings.playing);
	...
	if(!gameSettings.enableFixedResult){
		...
		if(!gameData.bonusRound){
			dropPhysicsBall(0, gameData.targetDrop.x + randomX, gameData.targetDrop.y);
			pausedPhysics(0, false);
		}else{
			dropPhysicsBall(1, gameData.targetDrop.x + randomX, gameData.targetDrop.y);
		...
		gameData.targetShapeBall.x = gameData.targetDrop.x;
		gameData.targetShapeBall.y = gameData.targetDrop.y;
		generateMovePath();
	}
...
function findMovePath(targetBall, row){
	...
	if(gameData.path.length > 0){
		selectedColumn = gameData.path[row].c;
	}
...
function checkWinPoint(){
	if(gameData.dropCon && !gameData.resultCon){
		...
		gameData.winPoint = plinkoSettings.prizes[gameData.resultIndex].value;
		...
		if(!gameSettings.gamePlayType){
			gameData.winPoint = gameData.winPoint * playerData.bet;
			playerData.bet = 0;
		}
		...
		if(bonusRound){
			...
			if(gameData.winPoint > 0){
				gameData.winPoint += gameData.extraCoin;
			...
			updateStat();
			toggleGameStat(textStrings.bonusRound);
			displayBonusText();
		}else if(gameData.winPoint > 0){
			gameData.winPoint += gameData.extraCoin;
			...
			//no win
			toggleGameStat(textStrings.lose);
			playSound('soundLose');
			updateStat();
		}
...
function checkGameEnd(){
	...
	if(gameSettings.gamePlayType){
		if(playerData.chance <= 0){
			toggleGameStat(textStrings.gameOver);
			toggleDropArea(false);
